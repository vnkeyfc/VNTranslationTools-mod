...
namespace VNTextPatch.Shared.Scripts.Artemis {
    internal static class LuaParser {
        public static ILuaNode Read(string text, ref int pos) {
            ...
            if (c == 't' && pos + 4 < text.Length && text.Substring(pos, 4) == "true") {
                return ReadBoolean(text, ref pos, true);
            }

            if (c == 'f' && pos + 5 < text.Length && text.Substring(pos, 5) == "false") {
                return ReadBoolean(text, ref pos, false);
            }
        ...
        private static LuaBoolean ReadBoolean(string text, ref int pos, bool value) {
            if (value == true) { 
                pos += 5;
